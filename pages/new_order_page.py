...
from PySide6.QtWidgets import (
    ...
    QAbstractSpinBox, QHBoxLayout, QFileDialog, QMessageBox, QListView, QApplication, QToolButton, QStyleOptionSpinBox, QStyle
)
from PySide6.QtGui import QStandardItemModel, QStandardItem, QMouseEvent, QKeyEvent, QCursor
from PySide6.QtCore import QDate, Qt, QEvent, QRect
...
# ============================================================================
# --- CLASSI PERSONALIZZATE PER BLOCCO ROTELLA, MULTI-SELEZIONE E CLICK ---
# ============================================================================
...
class NoWheelDateEdit(QDateEdit):
    ...
    def __init__(self, parent=None):
        ...
        # Installa il filtro eventi sul lineEdit per intercettare i click sul testo
        self.lineEdit().installEventFilter(self)
    
    def wheelEvent(self, event):
        """Ignora lo scroll della rotella."""
        event.ignore()
    ...
    def open_popup(self):
        """Simula programmaticamente un click sulla freccetta per aprire il calendario."""
...
class NoWheelComboBox(QComboBox):
    """
    ComboBox che ignora la rotella del mouse.
    Si apre cliccando su qualsiasi punto del widget.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def mousePressEvent(self, event):
        """Apre il popup al click sinistro su qualsiasi parte del box."""
        if event.button() == Qt.LeftButton:
            self.showPopup()
        else:
            super().mousePressEvent(event)
...
class CheckableComboBox(QComboBox):
    """
    ComboBox che permette la selezione multipla tramite Checkbox.
    Ignora la rotella e si apre cliccando sul testo.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def update_display_text(self):
        ...
        text_result = ", ".join(items_text)
        self.lineEdit().setText(text_result)
    ...
    def set_checked_items_from_string(self, text_string):
        ...
        selected_items = [x.strip() for x in text_string.split(',')]
        found_any = False
        for i in range(self.model.rowCount()):
            item = self.model.item(i)
...
class NewOrderPage(QWidget):
    ...
    def __init__(self, on_back):
        ...
        for item in ["Mandorla", "Cioccolato", "Ciocopassion", "Snob", "Stella"]:
            self.confetti_combo.addItem(item)
        ...
        self.acconto1_tipo_combo = NoWheelComboBox()
        ...
        self.acconto2_tipo_combo = NoWheelComboBox()
        ...
        self.table.setHorizontalHeaderLabels([
            "Ditta", "Codice", "Descrizione", "Quantità", "Prezzo Unitario", "Prezzo Totale"
        ])
    ...
    def add_row_with_data(self, data):
        ...
        self.table.setItem(row, 3, QTableWidgetItem(data.get("quantita", "0")))
        self.table.setItem(row, 4, QTableWidgetItem(data.get("prezzo_unitario", "0")))
    ...
    def update_totals(self, item):
        ...
        unit_price_item = self.table.item(row, 4)
    ...
    def save_order_and_close(self):
        ...
        if final_data: 
            QMessageBox.information(self, "Salvataggio Riuscito", f"Ordine salvato in:\n{target_file_path}")
            self.prepare_new_order()
    ...
    def save_order_and_print(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Errore di Stampa", f"L'ordine è stato salvato, ma la stampa è fallita:\n{e}")
        self.prepare_new_order()
    
    def save_order(self):
        pagamento_selezionato = self.payment_type.currentText()
        tipo_confetti_str = self.confetti_combo.lineEdit().text()
        ...
        order_info = {
            ...
            "tipo_confetti": tipo_confetti_str,
        ...
        for row in range(self.table.rowCount()):
            ...
            quantity = self.table.item(row, 3).text() if self.table.item(row, 3) else "0"
            unit_price = self.table.item(row, 4).text() if self.table.item(row, 4) else "0"
        ...
        if not customer_info.get("nome_cliente", "").strip():
            QMessageBox.warning(self, "Dati Mancanti", "Inserire almeno il 'Nome Cliente' prima di salvare.")
            return None, None 
        ...
        if not safe_cust_name: safe_cust_name = "Ordine"
            ...
            if old_file_to_remove and old_file_to_remove != target_file_path:
                if os.path.exists(old_file_to_remove):
                    try:
                        ...
                        print(f"Attenzione: salvataggio riuscito, ma impossibile rimuovere vecchio file: {e}")
